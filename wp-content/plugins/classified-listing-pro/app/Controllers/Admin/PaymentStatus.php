<?php

namespace Rtcl\Controllers\Admin;


use Rtcl\Helpers\Functions;
use Rtcl\Models\Payment;
use Rtcl\Models\RtclEmail;

/**
 * Class PaymentStatus
 *
 * @package Rtcl\Controllers\Admin
 */
class PaymentStatus
{

    function __construct() {
        add_action('transition_post_status', array($this, 'transition_post_status'), 10, 3);
    }


    public function transition_post_status($new_status, $old_status, $post) {

        if (rtcl()->post_type_payment !== $post->post_type) {
            return;
        }

        // TODO : need to add some logic

        if ('rtcl-completed' === $new_status && 'rtcl-completed' !== $old_status) {
            $payment = rtcl()->factory->get_order($post->ID);

            if ($payment && (empty($payment->pricing->getType()) || "regular" === $payment->pricing->getType())) {
                $listing = rtcl()->factory->get_listing($payment->get_listing_id());
                if (!absint(get_post_meta($post->ID, '_applied', true)) && in_array($listing->get_status(), ['publish', 'rtcl-expired'], true) && $visible = absint($payment->pricing->getVisible())) {
                    $promotions = [];
                    $do_update_status_to_publish = false;
                    if ($payment->pricing->hasPromotion() && $visible) {
                        // Featured Update
                        if ($payment->pricing->hasFeatured()) {
                            $promotions['featured'] = $visible;
                        }

                        // Top Update
                        if ($payment->pricing->hasTop()) {
                            $promotions['_top'] = $visible;
                        }

                        if ($payment->pricing->hasBumpUp()) {
                            $promotions['_bump_up'] = $visible;
                        }

                    } else {
                        $do_update_status_to_publish = true;
                        $current_date = new \DateTime(current_time('mysql'));
                        $current_date->add(new \DateInterval("P{$visible}D"));
                        $expDate = $current_date->format('Y-m-d H:i:s');
                        update_post_meta($payment->get_listing_id(), 'expiry_date', $expDate);
                    }
                    $promotions_status = Functions::update_listing_promotions($payment->get_listing_id(), $promotions);
                    // Check if post expired , then turn it to published
                    if ("rtcl-expired" === $listing->get_status() && $do_update_status_to_publish && !empty($promotions_status)) {
                        wp_update_post(array(
                            'ID'          => $listing->get_id(),
                            'post_status' => 'publish'
                        ));
                    }

                    update_post_meta($payment->get_id(), '_applied', 1);

                    // Hook for developers
                    do_action('rtcl_payment_completed', $payment);

                }
            }


            // send emails
            if (Functions::get_option_item('rtcl_email_settings', 'notify_users', 'order_completed', 'multi_checkbox')) {
                rtcl()->mailer()->emails['Order_Completed_Email_To_Customer']->trigger($payment->get_id(), $payment);
            }

            if (Functions::get_option_item('rtcl_email_settings', 'notify_admin', 'order_completed', 'multi_checkbox')) {
                rtcl()->mailer()->emails['Order_Completed_Email_To_Admin']->trigger($payment->get_id(), $payment);
            }

        }

    }

}