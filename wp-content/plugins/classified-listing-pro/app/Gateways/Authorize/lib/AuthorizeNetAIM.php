...
namespace Rtcl\Gateways\Authorize\lib;

use AuthorizeNetException;
use Rtcl\Gateways\Authorize\lib\shared\AuthorizeNetRequest;
...
class AuthorizeNetAIM extends AuthorizeNetRequest {
     ...
     * @param $payment
     */
    public function addLineItem($payment)
    {
        $line_item = "";
        $delimiter = "";
        $lineItemData = array(
            'id'          => $payment->get_listing_id(),
            'name'        => $payment->get_listing_title(),
            'description' => $payment->get_listing_title(),
            'quantity'    => 1,
            'unit_price'  => $payment->get_total(),
            'taxable'     => 0
        );
        $lineItemData = apply_filters('rtcl_authorizenet_line_item', $lineItemData, $payment);
    ...
    {
        return new AuthorizeNetAIM_Response($response, $this->_x_post_fields['delim_char'],
            $this->_x_post_fields['encap_char'], $this->_custom_fields);
    }
