...
class GatewayPaypal extends PaymentGateway
{
    ...
    protected function payment_status_completed($payment, $posted) {
        ...
        if ('completed' === $posted['payment_status'] || 'pending' === $posted['payment_status']) {
            ...
            $this->payment_complete($payment, !empty($posted['txn_id']) ? Functions::clean($posted['txn_id']) : __('IPN payment completed', 'classified-listing'));
        } else {
            if ('authorization' === $posted['pending_reason']) {
                $log->info('Aborting, Order #' . $payment->get_id() . ' is Authorization.');
                $this->payment_on_hold($payment, __('Payment authorized. Change payment status to processing or complete to capture funds.', 'classified-listing'));
            } else {
